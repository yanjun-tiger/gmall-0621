...
object GmvApp {
  def main(args: Array[String]): Unit = {
    ...
    val orderInfoDStream: DStream[OrderInfo] = kafkaDStream.map(record => {
      ...
      val tuple: (String, String) = consignee_tel.splitAt(4) //以索引位置4划分元组
    ...
    orderInfoDStream.foreachRDD(rdd => {
      println("开始往hbase里写入数据")
      rdd.saveToPhoenix("GMALL200621_ORDER_INFO",
        classOf[OrderInfo].getDeclaredFields.map(_.getName.toUpperCase()), //转变大写
